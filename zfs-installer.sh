...
# Updated by ServerMatter
# This script which is provided free of charge is provided with zero warranty.  It is destructive by nature and you are using it at your own risk and peril.

export NEEDRESTART_SUSPEND="false"
# If a mirror we use the entire drive. However if a raidz we restrict the size of the rpool
# This number will be divided by the number of drives in the raid and
# each drive will get that increment so the total of all drives = RAIDZ_RPOOL_SIZE_GB
# using 800 allows for a 2 drive loss via Raid-z2 and should give us 500+GB of useable Root storage
# Note: This rpool size will be split up over whatever the total number of drives is. So 6 drives = 75GB each.
export RAIDZ_RPOOL_SIZE_GB="800"
# we ALWAYS install to partition 4 no matter what kind of Mirror, RaidZ etc it is
export ROOT_PARTITION_NUMBER=4

## Prevent the execution of the script if the user has no root privileges
ROOT_UID=0   # Only users with $UID 0 have root privileges.
E_NOTROOT=87 # Non-root exit error.
if [ "${UID:-$(id -u)}" -ne "$ROOT_UID" ]; then
    echo 'Error: root privileges are needed to run this script - please run as sudo ./pre-install-script.sh'
    exit $E_NOTROOT
fi

echo "IF YOU GET AN ERROR RUNNING THIS SCRIPT SAYING - bash: zfs-installer.sh: line 31: syntax error near unexpected token - YOU'VE GOT WINDOWS CHARS IN YOUR SCRIPT - run dos2unix on it"

####### SSH ACCESS
# This first section creates an SSH access within the temporary install environment

sudo useradd -gusers -s/bin/bash -d/home/johndoe -m johndoe
echo 'johndoe:yourpassword' | chpasswd
echo "AllowUsers johndoe " >> /etc/ssh/sshd_config
...
echo ""
echo ""
read -p "$(echo -e 'If not already done, we have just added the SSH server and user johndoe/yourpassword\n\nDo you want to run the entire ZFS Install Script ?  yes/no ')" yn

case $yn in
yes )
   echo "";
   echo ok, proceeding to run the script;;
no )
   echo "";
   echo ok, we only added the SSH user.  You can now ssh into this server at the ip addr shown below using johndoe / yourpassword;
   echo ""
...
* )
   echo "";
   echo ok, since you did not answer, we only added the SSH user.;
   echo You can now ssh into this server at the ip addr shown below using johndoe / yourpassword;
   echo ""
   ip addr | grep "inet 192.168";
   echo "";
   exit;;
...
## RUNNING THE ZFS INSTALL SCRIPT

# install pre-requisite
NEEDRESTART_MODE=a apt install -y libwww-perl
...
# generates a 5 character randomized id for the root and boot pools
# WE HAVE DECIDED TO HARD CODE rpool_sm and bpool_sm instead of RANDOMIZE RPOOL and BPOOL as it would be such a rare use case for a problem

v_boot_partition_size=10G       # Integer number with `M` or `G` suffix
v_bpool_create_options=      # array; see defaults below for format
v_passphrase=
v_root_password=             # Debian-only
v_rpool_name=rpool           # Jay - you have experienced if you change rpool and bpool to anything else, the script fails upon the point of the first live boot looking for that named pool that does not exit
...
# etc directory was added by Jay on 2023.01.12 but we found problems with mariadb and possibly others
# so  we removed /etc
# good poliicy will be to simply copy those contents to a safew placce for added protection
...
function display_intro_banner {
  local dialog_message='Hello ServerMatter Installation Tech:

This script will, among other things
* identify your disks
* prepare the ZFS pools on the system
* add Ubuntu universe repo
* apt update Ubuntu
* and configure the boot
...
function register_exit_hook {
  function _exit_hook {
    ...
    #
#    echo "Currently set exports, for performing an unattended (as possible) installation with the same configuration:"
#    echo "export ZFS_USE_PPA=$v_use_ppa"
#    echo "export ZFS_SELECTED_DISKS=$(IFS=,; echo -n \"${v_selected_disks[*]}\")"
#    v_vdev_configs_export=$(declare -p v_vdev_configs | perl -pe "s/.*?\((.+)\)/\1/")
#    echo "export ZFS_VDEV_CONFIGS='$v_vdev_configs_export'"
#    echo "export ZFS_BOOT_PARTITION_SIZE=$v_boot_partition_size"
#    echo "export ZFS_PASSPHRASE=$(printf %q "$v_passphrase")"
#    echo "export ZFS_DEBIAN_ROOT_PASSWORD=$(printf %q "$v_root_password")"
#    echo "export ZFS_RPOOL_NAME=$v_rpool_name"
#    echo "export ZFS_BPOOL_CREATE_OPTIONS=\"${v_bpool_create_options[*]}\""
#    echo "export ZFS_RPOOL_CREATE_OPTIONS=\"${v_rpool_create_options[*]}\""
#    echo "export ZFS_NO_INFO_MESSAGES=1"
#    echo "export ZFS_SWAP_SIZE=$v_swap_size"
#    echo "export ZFS_FREE_TAIL_SPACE=$v_free_tail_space"

    echo "A If it exited the script at this point just rerun it.  Not sure what the bug is yet"

    set -x
  }
  echo "B If it exited the script at this point just rerun it.  Not sure what the bug is yet"
...
function update_apt_index {
  # apt -y update
  NEEDRESTART_MODE=a apt -y update
}
...
function install_host_base_packages {
  ...
  NEEDRESTART_MODE=a apt install -y efibootmgr dialog software-properties-common
}
...
"
  ...
  # this tells us how many disks are part of the array
  # this will be important in raidz configs as we divide the RAIDZ_RPOOL_SIZE_GB / NUM_SELECTED_DISKS to know how large each
  # partition should be per disk to = our total rpool desired size
  export NUM_SELECTED_DISKS=${#v_selected_disks[@]}
...
function select_vdev_configs {
  if [[ -n ${ZFS_VDEV_CONFIGS:-} ]]; then
    ...
    # New code: Initialize a variable to track RAID configuration
    local is_raid_other_than_mirror=false
    local raid_type=false
    while true; do
...
If any disks are remaining, they will be made available for another group."
    ...
    # New code: Check if STRIPE, MIRROR RAIDZ1, RAIDZ2, or RAIDZ3 is selected
    #echo "The value of current_vdev_type is: $current_vdev_type"
    #echo "Press enter to continue..."
    #read -r  # The script will pause here and wait for the user to press enter

      fi
    done

    # New code: create an environment variable available everywhere which tells us which vdev type we are using
    export CURRENT_VDEV_DISK_ARRAY_TYPE=$current_vdev_type
  fi
}
...
function install_host_zfs_packages {
  if [[ $v_use_ppa == "1" ]]; then
    if [[ ${ZFS_SKIP_LIVE_ZFS_MODULE_INSTALL:-} != "1" ]]; then
      ...
      NEEDRESTART_MODE=a apt install --yes libelf-dev zfs-dkms
  ...
  #
  NEEDRESTART_MODE=a apt install --yes zfsutils-linux
...
function install_host_zfs_packages_Debian {
  if [[ ${ZFS_SKIP_LIVE_ZFS_MODULE_INSTALL:-} != "1" ]]; then
    ...
    NEEDRESTART_MODE=a apt install --yes -t buster-backports zfs-dkms
...
function install_host_zfs_packages_UbuntuServer {
  if [[ $v_use_ppa != "1" ]]; then
    NEEDRESTART_MODE=a apt install --yes zfsutils-linux efibootmgr
    ...
    #
    NEEDRESTART_MODE=a apt -y update
    NEEDRESTART_MODE=a apt install --yes "linux-headers-$(uname -r)"

    install_host_zfs_packages
  else
    NEEDRESTART_MODE=a apt install --yes efibootmgr
  fi
}
...
function setup_partitions {
  ...
  for selected_disk in "${v_selected_disks[@]}"; do
    ...
    # Debug: Display the value of $CURRENT_VDEV_DISK_ARRAY_TYPE and wait for the user to press enter
    #echo "The value of CURRENT_VDEV_DISK_ARRAY_TYPE is: $CURRENT_VDEV_DISK_ARRAY_TYPE"
    #echo "Press enter to continue..."
    #read -r  # The script will pause here and wait for the user to press enter

    # New code: Conditional logic for rpool size based on RAID configuration
    if [[ "$CURRENT_VDEV_DISK_ARRAY_TYPE" =~ .*raidz.* ]]; then
      # to divide the announced rpool size x the number of disks
        rpool_gb_per_disk=$((RAIDZ_RPOOL_SIZE_GB / NUM_SELECTED_DISKS))
        # Set rpool size to rpool_gb_per_disk GB when RAIDZ1, RAIDZ2, or RAIDZ3 is selected
        sgdisk -n3:0:+${rpool_gb_per_disk}G -t3:BF01 "$selected_disk" # Root pool
        echo "ONE: sgdisk -n3:0:+${rpool_gb_per_disk}G -t3:BF01 $selected_disk"
    else
        # Original line for root pool creation which is for mirrors
        # It will insure the entire remaining space is used for the root pool
        sgdisk -n3:0:"-${required_tail_space}G" -t3:BF01 "$selected_disk" # Root pool
        echo "TWO: sgdisk -n3:0: -${required_tail_space}G -t3:BF01 $selected_disk"
    fi
        # New code ends here
        sgdisk -n4:0:0 -t4:8300 "$selected_disk" # Temporary partition
  done
    # Debug: View if any errors
    #echo "View above and see if any errors after sgdisk just ran. We hope to see sgdisk -n3:0:+${rpool_gb_per_disk}G "
    #echo "Press enter to continue..."
    #read -r  # The script will pause here and wait for the user to press enter

  # The partition symlinks are not immediately created, so we wait.s
  #
...
'
  ...
  #
  echo "DEBUG: LINE #1219 REMOUNTING"
...
function install_operating_system_UbuntuServer {
  ...
  local dialog_message='ZFS ROOT MIGRATION SCREEN

=== DO NOT - I REPEAT - DO NOT - PRESS THE OK BUTTON UNTIL AFTER UBUNTU SERVER IS INSTALLLED ===

THESE ARE YOUR NEXT STEPS IN EXACTLY THIS ORDER:

1) Return to the Ubuntu Server installer by pressing Ctrl+Alt+F1.
** Then proceed with the configuration as usual.

2) When the update option is presented, choose to update Ubuntu to the latest version.

3) When you reach the - Guided Storage configuration - screen:

  select `(x) Custom storage layout` -> `Done`
  select `'"$selected_disk_subiquity_name"'`/`"PARTITION '"$ROOT_PARTITION_NUMBER"'` -> `Edit`
    Note: if this is a mirror - select the first of the disks labeled `'"$selected_disk_subiquity_name"'`
    set `Format:` to `ext4` (mountpoint will be automatically selected)
    click `Save`
  click `Done` -> `Continue` (ignore warning)
  follow through the installation, until the end (about  15 minutes including updates)

4) After the above is completed it will say `Reboot` === DO NOT REBOOT ===

  Instead switch back to this ZFS ROOT MIGRATION SCREEN using Ctrl+Alt+F2

  Only AFTER you have switched back to this screen do you Press OK below to allow this script to copy the Parition 4 contents to your new ZFS filesystem

Note: you can toggle to the Ubuntu Installer and this ZFS ROOT MIGRATION SCREEN as needed with Ctrl+Alt+F1 or F2

=== DO NOT PRESS OK BELOW UNLESS ALLL STEPS ABOVE ARE COMPLETED AND UBUNTU IS FULLY INSTALLED ON EXT4 ===
'

  whiptail --msgbox "$dialog_message" 40 100
...
function create_pools_and_datasets {
  ...
  echo "DEBUG: LINE 1375 ABOUT TO zpool create using c_zfs_mount_dir $c_zfs_mount_dir";
  ...
  zpool create \
   ...
   # Debug: Display the value of rpool_create_vdev_options and wait for the user to press enter
    #echo "rpool_create_vdev_options = ${rpool_create_vdev_options[@]}"
    #echo "Press enter to continue..."
    #read -r  # The script will pause here and wait for the user to press enter
  ...
  echo "DEBUG: LINE 1427 ABOUT TO zpool create using c_zfs_mount_dir $c_zfs_mount_dir";
  zpool create \
...
function copy_zpool_cache {
  echo "DEBUG: LINE 1450 ABOUT TO mkdir $c_zfs_mount_dir/etc/zfs using c_zfs_mount_dir $c_zfs_mount_dir";
...
function sync_os_temp_installation_dir_to_rpool {
  ...
  # this was set my  Jay / CompuMatter on 2023.01.12  to disable kernel restart warning
  # it is untested and may either fail or rsync below may make useless
  sed -i 's/#$nrconf{kernelhints} = -1;/$nrconf{kernelhints} = 0;/' /etc/needrestart/needrestart.conf

  echo "DEBUG: LINE 1477 ABOUT TO rsync using c_zfs_mount_dir $c_zfs_mount_dir";
  ...
  echo "DEBUG: LINE 1491 ABOUT TO mkdir $c_zfs_mount_dir/run using c_zfs_mount_dir $c_zfs_mount_dir";
...
function remove_temp_partition_and_expand_rpool {
  if (( v_free_tail_space < c_temporary_volume_size )); then
    ...
    #
    echo "DEBUG: LINE 1519 ABOUT TO import $c_zfs_mount_dir to $v_rpool_name using c_zfs_mount_dir $c_zfs_mount_dir";
...
function prepare_jail {
  ...
  echo "DEBUG: LINE 1535 ABOUT TO mount $c_zfs_mount_dir/$virtual_fs_dir using c_zfs_mount_dir $c_zfs_mount_dir";
...
function install_jail_zfs_packages {
  if [[ $v_use_ppa == "1" ]]; then
    ...
    chroot_execute "NEEDRESTART_MODE=a apt -y update"

    chroot_execute 'echo "zfs-dkms zfs-dkms/note-incompatible-licenses note true" | debconf-set-selections'

    chroot_execute "NEEDRESTART_MODE=a apt install --yes libelf-dev zfs-initramfs zfs-dkms"
  else
    ...
    #
    chroot_execute "NEEDRESTART_MODE=a apt install --yes zfs-initramfs zfs-zed zfsutils-linux"
  fi
}
...
APT'
  ...
  chroot_execute 'echo "zfs-dkms zfs-dkms/note-incompatible-licenses note true" | debconf-set-selections'
  chroot_execute "NEEDRESTART_MODE=a apt install --yes zfs-initramfs zfs-dkms"
}

function install_jail_zfs_packages_UbuntuServer {
  if [[ $v_use_ppa != "1" ]]; then
    chroot_execute "NEEDRESTART_MODE=a apt install --yes zfsutils-linux zfs-initramfs"
  else
...
function fix_filesystem_mount_ordering {
  ...
  echo "DEBUG: LINE 1709 c_zfs_mount_dir $c_zfs_mount_dir";
  if [[ ! -s $c_zfs_mount_dir/etc/zfs/zfs-list.cache/$c_bpool_name || ! -s $c_zfs_mount_dir/etc/zfs/zfs-list.cache/$v_rpool_name ]]; then
    ...
    echo "DEBUG: LINE 1723 c_zfs_mount_dir $c_zfs_mount_dir";
  ...
  echo "DEBUG: LINE 1743 c_zfs_mount_dir $c_zfs_mount_dir";
...
function prepare_for_system_exit {
  # Attempt to unmount the virtual filesystems
  for virtual_fs_dir in dev sys proc; do
    echo "DEBUG: LINE 1757 unmounting $c_zfs_mount_dir/$virtual_fs_dir while c_zfs_mount_dir = $c_zfs_mount_dir"
    if ! umount --recursive --force --lazy "$c_zfs_mount_dir/$virtual_fs_dir"; then
      echo "Unmounting $c_zfs_mount_dir/$virtual_fs_dir failed, checking for active processes..."
      lsof | grep "$c_zfs_mount_dir/$virtual_fs_dir"
      fuser -vm "$c_zfs_mount_dir/$virtual_fs_dir"
    fi
  done

  # Wait for virtual filesystems to unmount
  local max_unmount_wait=30
  ...
  # If filesystems are still mounted, check what's using them and try to unmount again
  for virtual_fs_dir in dev sys proc; do
    if mountpoint -q "$c_zfs_mount_dir/$virtual_fs_dir"; then
      echo "Filesystem $virtual_fs_dir is still in use. Checking for processes that might be using it..."

      # List open files in the mount
      lsof +D "$c_zfs_mount_dir/$virtual_fs_dir"

      # List processes using the file
      fuser -vm "$c_zfs_mount_dir/$virtual_fs_dir"

      echo "Attempting to kill any processes holding the mount..."
      fuser -km "$c_zfs_mount_dir/$virtual_fs_dir" || true

      echo "Re-issuing umount for $c_zfs_mount_dir/$virtual_fs_dir"
      umount --recursive --force --lazy "$c_zfs_mount_dir/$virtual_fs_dir" || true
    fi
  done

  # Finally, export the ZFS pools
  zpool export -a
}
